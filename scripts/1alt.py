...
class Edrone():
	def __init__(self):
		...
		#self.fimal_position = [0.0, 0.0, 0.0]
	...
	def throttle_set_pid(self,throttle):
		self.Kp[2] = throttle.Kp * 0.6
		self.Ki[2] = throttle.Ki * 0.8
		self.Kd[2] = throttle.Kd * 0.3
		print("Khhui",self.kp[2])
	...
	def pid(self):
		...
		#print(self.error[0],self.error[1],self.error[2])
		

		#proportional error
		self.roll_kp = self.Kp[0] * self.error[0]
		self.pitch_kp = self.Kp[1] * self.error[1]
		self.throttle_kp = self.Kp[2] * self.error[2]
		print("KP::::",self.throttle_kp)

		#derivative error
		self.roll_kd = -self.Kd[0]*(self.error[0] - self.prev_error[0])   
		self.pitch_kd = self.Kd[1] *(self.error[1] - self.prev_error[1]) 
		self.throttle_kd = self.Kd[2] *(self.error[2] - self.prev_error[2])
		print("KD::::::::",self.throttle_kd)
		...
		self.out_throttle = self.throttle_kp + self.throttle_kd + self.iterm[2]
		#print("output",self.out_roll,self.out_pitch,self.out_throttle)
