  ...
  #    self.drone_position[0] != self.drone_setpoint3[0] and self.drone_position[1] != self.drone_setpoint3[1]
	...
	if(self.count == 1):
		...
		if(self.state == 1):
			...
			self.p_x = self.x_pid[0] * self.error[0]
			self.i_x = self.x_pid[1] * 0.01 * self.error[0] + self.i_term[0]
	       		self.d_x = self.x_pid[2] * 100 * (self.error[0] - self.prev_values[0])

			self.p_y = self.y_pid[0] * self.error[1]
			self.i_y = self.y_pid[1] * 0.01 * self.error[1] + self.i_term[1]
	       		self.d_y = self.y_pid[2] * 100 * (self.error[1] - self.prev_values[1])
		...
		if(self.state == 0):
			...
			self.p_x = self.x_pid[0] * self.error[0]
			self.i_x = self.x_pid[1]* 0.01 * self.error[0] + self.i_term[0]
	       		self.d_x = self.x_pid[2]* 10 * (self.error[0] - self.prev_values[0])

			self.p_y = self.y_pid[0] * self.error[1]
			self.i_y = self.y_pid[1] * 0.01 * self.error[1] + self.i_term[1]
	       		self.d_y = self.y_pid[2] * 10 * (self.error[1] - self.prev_values[1])
