...
class Edrone():
    ...
    def __init__(self):
  	...
  	self.error = [0.0, 0.0, 0.0]					# storing updated errors in each axis [pitch,roll, yaw]
        self.iterm = [0.0, 0.0, 0.0]					# integral term
	...
	# Compute error in each axis [pitch,roll, yaw]
        ...
        self.pitch_kp = self.Kp[0] * self.error[0]
        self.roll_kp = self.Kp[1] * self.error[1]
        self.yaw_kp = self.Kp[2] * self.error[2]

        #derivative error [pitch,roll, yaw]
        self.pitch_kd = self.Kd[0]*(self.error[0] - self.prev_error[0])   
        self.roll_kd = self.Kd[1] *(self.error[1] - self.prev_error[1]) 
	...
	# Calculate total pid output required for each axis
        self.out_pitch = self.pitch_kp + self.pitch_kd + self.iterm[0]
	self.out_roll = self.roll_kp + self.roll_kd + self.iterm[1]
        self.out_yaw = self.yaw_kp + self.yaw_kd + self.iterm[2]

	# Equations to compute the pwm for each propeller.
	...
	# Limit the output value between the maximum(0) and minimum(1024)range
