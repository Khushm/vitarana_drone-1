...
class Edrone():
    ...
    def __init__(self):
        ...
        self.sample_time = 0.60  # in seconds

        # Publishing /edrone/pwm, /roll_error, /pitch_error, /yaw_error
        self.pwm_pub = rospy.Publisher('/edrone/pwm', prop_speed, queue_size=10)
        self.roll_error_pub = rospy.Publisher('/roll_error',PidTune, queue_size=10) 
        self.pitch_error_pub = rospy.Publisher('/pitch_error', PidTune, queue_size=10) 
        self.yaw_error_pub = rospy.Publisher('/yaw_error', PidTune, queue_size=10)
        self.zero_error_pub = rospy.Publisher('/zero_error', PidTune, queue_size=10)
        self.z_error_pub = rospy.Publisher('/z_error', PidTune, queue_size=10)
        self.z_error_pub = rospy.Publisher('/throttle_error', PidTune, queue_size=10)
    ...
    def pitch_set_pid(self, pitch):
        ...
        self.Kd[1] = pitch.Kd * 0.00001

    def yaw_set_pid(self, yaw):
        self.Kp[2] = yaw.Kp * 0.09  # This is just for an example. You can change the ratio/fraction value accordingly
        self.Ki[2] = yaw.Ki * 0.004
        self.Kd[2] = yaw.Kd * 0.001

    def throttle_set_pid(self, throttle):
        self.Kp[3] = throttle.Kp * 0.09  # This is just for an example. You can change the ratio/fraction value accordingly
        self.Ki[3] = throttle.Ki * 0.004
        self.Kd[3] = throttle.Kd * 0.001
...
# ----------------------------Define callback function like roll_set_pid to tune pitch, yaw--------------
	...
	#print(self.setpoint_euler[0],self.setpoint_euler[1],self.setpoint_euler[2], self.out_roll, self.out_pitch, self.out_yaw)
	...
	self.roll_error_pub.publish(self.error[0],self.error[0],self.error[0],self.error[0])
